...
def create_labels(anno_path, project_folder, filename):
    """
    ...
    if os.path.exists(anno_path):
        ...
        if required_columns.issubset(df.columns):
            df['ElementType'] = df['ElementType'].apply(lambda x: x if x in classes[:-1] else 'Others')
            # All Unique labels
    ...
    for root, _, files in os.walk(input_dir):
        for file in files:
            if file.endswith('.csv'):
                ...
                create_labels(file_path, annot_folder, out_filename)
