...
def preprocess(df):
    ...
    for fig_i, (floor, floor_df) in enumerate(split_dfs.items()):
        ...
        print(f"Floor has {floor_df.shape} points")
        all_lines[floor] = lines
        floor_df.loc[:, 'is_wall'] = -1
        floor_df.loc[:, 'is_floor'] = -1
        ...
        if lines is not None:
            ...
            for idx, line in enumerate(lines):
                ...
                # Assign the current line's index (or any unique label) as the wall_label
                floor_df.loc[close_points, 'is_wall'] = idx


        floor_z = floor_df['z'].min()
        # if point z is within proximity of the floor_z, then it is a floor

        floor_df.loc[(floor_df['z'] >= floor_z - proximity) & (floor_df['z'] <= floor_z + proximity), 'is_floor'] = 1
...
def vote_cluster(df):
    ...
    cluster_votes = df.groupby("cluster")["pred_l"].value_counts().reset_index(name="count")

    # Find the label with the most votes for each cluster
    cluster_votes = cluster_votes.loc[
        cluster_votes.groupby("cluster")["count"].idxmax()
    ]

    # Assign the winning label to each cluster
    cluster_votes = cluster_votes.set_index("cluster")["pred_l"]
...
def postprocess(df):
    ...
    df[['r', 'g', 'b', 'pred_l']] = df[['r', 'g', 'b', 'pred_l']].astype(int)  # Changed 'label' to 'l'
    ...
    df['Label_Name'] = df['pred_l'].map(inverse_label_mapping)
