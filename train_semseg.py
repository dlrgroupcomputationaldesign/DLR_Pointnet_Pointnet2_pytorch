...
def run(args):
    ...
    TRAIN_DATASET = DLRGroupDataset(split='train', data_type=args.data_type,
                                    data_root=root, num_point=NUM_POINT,
                                    test_project=args.test_project,
                                    block_size=2.0, sample_rate=1.0, transform=None)
    # TRAIN_DATASET = S3DISDataset(split='train', data_root=root, num_point=NUM_POINT, test_area=args.test_area, block_size=1.0, sample_rate=1.0, transform=None)
    print("start loading test data ...")
    TEST_DATASET = DLRGroupDataset(split='test', data_type=args.data_type,
                                   data_root=root, num_point=NUM_POINT,
                                   test_project=args.test_project,
                                   block_size=2.0, sample_rate=1.0, transform=None)
